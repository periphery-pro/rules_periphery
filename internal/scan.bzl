"""
Declares the implementation backing the public `scan` rule.
"""
...
def scan_impl(ctx):
    ...
    periphery = ctx.executable.periphery
    periphery_info = ctx.attr.periphery[DefaultInfo]
    ...
    return DefaultInfo(
        ...
        ).merge(periphery_info.default_runfiles),
    )

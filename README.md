...
`scan` selects a scanner built for the local host's OS and CPU because
`bazel run` launches it locally. This also applies to scans generated by
`scan_auto`. Scanned targets keep their configured target and execution
platforms, so they can still compile remotely. `scan_test` and `scan_report`
continue to select the scanner for their execution platform. Prebuilt archives
and local binaries must already be compatible with the platform where they run.
...
# Platform regression tests using portable scanner and compiler stand-ins.
./tests/platforms.sh
